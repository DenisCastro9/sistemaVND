...
{
    public partial class BuscarPedidoParaFactura : Form
    {
        ...
        public BuscarPedidoParaFactura()
        {
            InitializeComponent();
        ...
        //CARGAR LOS PEDIDOS EN EL DATAGRID en estado 4, REMITADOS
        private void cargarDataPedido()
        {
            conexion.Open();
            string sql = "select p.numero as numero, p.fechaDePedido as fechaDePedido, p.importeTotal as importeTotal, p.totalPares as totalPares, a.nombre as articulo, c.id as idCliente, c.razonSocial as cliente from pedido as p join cliente as c on p.idCliente = c.id join articulo as a on p.idArt = a.idarticulo  where  p.idEstadoPedido=4 order by numero asc";
            ...
            while (registros.Read())
            {
                ...
                dataGridView1.Rows.Add(registros["numero"].ToString(), //numero pedido 0
                                       ...
                                       registros["idCliente"].ToString(),//nombre cliente 5
                                       registros["cliente"].ToString());//nro cliente 6
            }
        ...
        {
            conexion.Open();
            string consulta = "select p.numero as numero, p.fechaDePedido as fechaDePedido, p.importeTotal as importeTotal, p.totalPares as totalPares, a.nombre as articulo, c.id as idCliente, c.razonSocial as cliente from pedido as p join cliente as c on p.idCliente = c.id join articulo as a on p.idArt = a.idarticulo  where p.idEstadoPedido=4 and c.razonSocial LIKE @busqueda OR p.numero LIKE @busqueda AND p.idEstadoPedido=4 order by numero asc";
        ...
        //BOTON ACEPTAR pasa datos al registrar factura
        private void button1_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                RegistrarFacturacion r = new RegistrarFacturacion(nro, total, idCliente);
                this.Hide();
                r.ShowDialog();
                this.Close();
        ...
        private void BuscarPedidoParaFactura_Load(object sender, EventArgs e)
        {
            dataGridView1.ClearSelection();
            numeroRegistros();
        }
